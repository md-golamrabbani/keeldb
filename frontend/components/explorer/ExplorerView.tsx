  ...
  useEffect(() => {
    api
      ...
      .then((cs) => {
        ...
        if (params.get("conn")) return;               // handled by the effect below
        const id = newWsId();
        setWorkspaces([{ id, label: "New workspace" }]);
        setActiveWs(id);
      })
      .catch((e) => setError(String(e)));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // "Explore this connection" deep-link (/explorer?conn=<id>). Explorer is now
  // mounted once and kept alive, so we can't rely on reading the param at mount:
  // react to it, focusing an existing workspace for that connection or opening a
  // new one. The ref makes each distinct conn value act once (guards re-runs).
  const connParam = params.get("conn");
  const handledConnRef = useRef("");
  useEffect(() => {
    if (!connParam || !connections.length) return;
    if (handledConnRef.current === connParam) return;
    if (!connections.some((c) => c.id === connParam)) return;
    handledConnRef.current = connParam;
    const sessions = useUiStore.getState().explorer?.sessions ?? {};
    const existing = workspaces.find((w) => sessions[w.id]?.connId === connParam);
    if (existing) { setActiveWs(existing.id); return; }
    const id = newWsId();
    setWorkspaces((prev) => [...prev, { id, label: "New workspace", initialConnId: connParam }]);
    setActiveWs(id);
  }, [connParam, connections]); // eslint-disable-line react-hooks/exhaustive-deps
